...
public class OrderAssert extends AbstractObjectAssert<OrderAssert, Order> {
  ...
  /**
   * Verifies that the actual Order's orderNumber is equal to the given one.
   * @param orderNumber the given orderNumber to compare the actual Order's orderNumber to.
   * @return this assertion object.
   * @throws AssertionError - if the actual Order's orderNumber is not equal to the given one.
   */
  public OrderAssert hasOrderNumber(Long orderNumber) {
    // check that actual Order we want to make assertions on is not null.
    isNotNull();

    // overrides the default error message with a more explicit one
    String assertjErrorMessage = "\nExpecting orderNumber of:\n  <%s>\nto be:\n  <%s>\nbut was:\n  <%s>";

    // null safe check
    Long actualOrderNumber = actual.getOrderNumber();
    if (!Objects.deepEquals(actualOrderNumber, orderNumber)) {
      failWithMessage(assertjErrorMessage, actual, orderNumber, actualOrderNumber);
    }
